...
if ! [ -x "$(command -v fast)" ]; then
        fast="docker run -v $(pwd):/e -w /e fasttool/fast fast"
else
        fast=$(command -v fast)
fi
...
stdout() {
	hash=$1
	shift
	assertSame $hash $($fast $@ | $sha256sum | awk '{print $1}')
} 
export -f stdout
...
stderr() {
	hash=$1
	shift
	assertSame $hash $($fast $@ 2>&1 > /dev/null | $sha256sum | awk '{print $1}')
} 
export -f stderr
...
stdouterr() {
	hash=$1
	shift
	assertSame $hash $($fast $@ 2>&1 | $sha256sum | awk '{print $1}')
} 
export -f stdouterr
...
testDiff() {
	...
	catout a0b0dea4e30f6b0a01a861dd70617f7e1c92e4768b35756a4bdede77773e4429 Hello.patch.xml
}
